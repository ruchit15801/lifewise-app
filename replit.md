...
## Data Model
- `Bill` type extended with: `reminderType` (bill/subscription/custom), `repeatType` (none/daily/weekly/monthly/yearly), `status` (active/paid/snoozed), `snoozedUntil`, `reminderDaysBefore`
...
- Storage keys use `_v2` suffix to avoid stale data from previous schema
...
- No light mode, no emoji - uses Ionicons exclusively
